...
{	
	...
	public class SectionBookcasePage extends SectionSprite
	{		
		...
		/*constructor：建構值*/
		public function SectionBookcasePage() : void
		{
			super(ENUM_SECTION.PAGE_BOOKCASE);
			
			this.tf_textField = new TextField();
			this.tf_textField.autoSize = TextFieldAutoSize.CENTER;
			this.tf_textField.text = "Bookcase Page";
		...
		public override function Display( a_flow : IProgressController = null ) : void
		{
			...
			param.vars.x = 0;
			if( a_flow != null )
				param.vars.onComplete = a_flow.Next();
			
			this.x = rect.width;
			this.visible = true;
			
			TweenMax.to( this, param.time, param.vars );
		}
		...
		public override function Disappear( a_flow : IProgressController = null ) : void
		{
			...
			param.vars.onCompleteParams = [this, a_flow];
			param.vars.onComplete = function( mc : DisplayObject, flow : IProgressController ) 
			{ 
				mc.visible = false;
				if( flow != null )
					flow.Next();
