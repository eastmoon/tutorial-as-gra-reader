	...
	{		
		...
		private function EventToBookcase( a_event : MouseEvent ) : void
		{
			...
			ApplicationFacade.getInstance().sendNotification( WSProgressFlow.EVENT_PROGRESS_FLOW, 
								 new WSPFController( ENUM_WINDOW.CONTENT, ENUM_SECTION.PAGE_BOOKCASE ));
		}
		private function EventToReader( a_event : MouseEvent ) : void
		{
			trace("Click Reader");
			ApplicationFacade.getInstance().sendNotification( WSProgressFlow.EVENT_PROGRESS_FLOW, 
								 new WSPFController( ENUM_WINDOW.CONTENT, ENUM_SECTION.PAGE_READER ));
		}
		private function EventToDownload( a_event : MouseEvent ) : void
		{
			trace("Click Download");
			ApplicationFacade.getInstance().sendNotification( WSProgressFlow.EVENT_PROGRESS_FLOW, 
								 new WSPFController( ENUM_WINDOW.CONTENT, ENUM_SECTION.PAGE_DOWNLOAD ));
		}
		private function EventToConfig( a_event : MouseEvent ) : void
		{
			trace("Click Config");
			ApplicationFacade.getInstance().sendNotification( WSProgressFlow.EVENT_PROGRESS_FLOW, 
								 new WSPFController( ENUM_WINDOW.CONTENT, ENUM_SECTION.PAGE_CONFIG ));
		}
